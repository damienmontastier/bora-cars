...
| `useContactProForm.ts` | Parcours « Leasing professionnel » : réponses des 5 étapes, validation par étape (revalidation à la saisie après un 1er essai ; l'envoi valide les 5 étapes), champs conditionnels vidés quand ils sont masqués, libellés d'options et placeholders (lus bruts via `tm` : « @ » casse le compilateur vue-i18n). Créé par `ElementsContactFormPro`, lu par ses étapes |
...
- `CONTACT_PRO_CONFIG.ts` — parcours Leasing pro de la page Contact : valeurs des listes + libellé EXACT de l'option Airtable correspondante, étapes, profils (`general` | `pro`), paramètre `profil`. Importé par le client ET par le serveur (`~/config/...`) ; jamais éditable depuis le Studio
...
- `contact.vue` — Contact page, deux onglets (`AtomsProfileSwitch`) :
  ...
  - **Leasing professionnel** (`ElementsContactFormPro`) : 5 étapes (A)–(E) reprises du prototype client `bora_test_form/`, une étape = un composant `contact-form/pro/Step*.vue`. Succès → `PageContactProSuccess` remplace la page.
  ...
  - Valeurs des listes pro = constantes `config/CONTACT_PRO_CONFIG.ts`, lues aussi par l'API (liste blanche → libellés Airtable exacts, `typecast: true`). Les libellés affichés sont dans le glossaire (`contact.pro.*`, `contact.profile.*`) ; les textes éditoriaux (accueil, écran de succès, liens) dans le singleton `contact` (`proIntro`, `proSuccess`). Détail CRM : `../docs/AIRTABLE.md`.
