...
| `useWhatsappMessage.ts` | Per-page WhatsApp prefill. A page calls `provideWhatsappMessage(computed(() => page.value?.whatsappMessage))`; `BaseLink` (the single link chokepoint) injects `?text=` into any `wa.me`/`whatsapp.com` URL it resolves via `withWhatsappText()`. Pages that don't provide a message (homepage, menu) keep a blank WhatsApp message. Sanity field `whatsappMessage` (tab "WhatsApp", static text) lives on the `professionnel`, `proprietaire` and `catalogue` singletons. The car page (`carPage`) instead uses a **template** with `{marque}`/`{modele}`/`{prix}`/`{periode}`/`{duree}`/`{quand}`/`{url}` tokens — a custom Studio field input (`studio/components/WhatsappTemplateInput.tsx`, set as `components.input` on the `carPage` `whatsappMessage` field — wraps the default `internationalizedArrayText` UI via `renderDefault`) shows the variables as click-to-insert / draggable chips; `Pricing.vue` fills the template (falling back to the `car.whatsappMessage.*` i18n default) and injects it via `withWhatsappText()`. |
