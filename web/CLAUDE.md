...
server/
  api/contact.post.ts           # formulaire de contact (2 parcours) → Airtable (server/utils/airtable.ts + contactPro.ts, token serveur)
...
| `useContactSwitchMotion.ts` | Page Contact, changement d'onglet : durées/eases communs au texte de gauche et au formulaire, `canAnimate()` (monté, preloader fini, pas de reduced-motion, élément visible), `track()` (contexte GSAP annulé au démontage) |
| `useContactProForm.ts` | Parcours « Leasing professionnel » : lit la config Sanity `contact.proForm` (étapes → champs), tient les réponses (clé = `role` pour les coordonnées, `consent`, sinon `_key` du champ), visibilité des champs conditionnels (`showIf`, réponse vidée quand le champ se masque), validation par étape (revalidation à la saisie après un 1er essai ; l'envoi valide toutes les étapes), `toPayload()` → `{ answers, questions }`. Créé par `ElementsContactFormPro`, lu par `pro/Step.vue` et `pro/Field.vue` |
...
- `CONTACT_PRO_CONFIG.ts` — page Contact : profils (`general` | `pro`), paramètre `profil`, `CONTACT_MAX_LENGTH`, et pour le Leasing pro les **types** de la config Sanity (`ProFormData`, un type par `_type` de champ) + la logique partagée client/serveur (`proAnswerKey`, `isProFieldVisible`, `isProFieldRequired`, `emptyProAnswer`…). Importé par le client ET par le serveur (`~/config/...`)
...
- `contact.vue` — Contact page, deux onglets (`AtomsProfileSwitch`) :
  ...
  - **Leasing professionnel** (`ElementsContactFormPro`) : **formulaire piloté par Sanity** (`contact.proForm`, onglet « Leasing professionnel » du Studio) — étapes, champs, choix, conditions d'affichage et tous les textes. Une étape = `contact-form/pro/Step.vue` (deux champs « demi-largeur » consécutifs → une ligne ; un champ conditionnel placé juste sous sa question → bloc en retrait), un champ = `contact-form/pro/Field.vue` (un rendu par `_type` : `proFieldIdentity`, `proFieldText`, `proFieldTextarea`, `proFieldChoice`, `proFieldYesNo`, `proFieldCheckbox`, `proFieldMonthYear`, `proFieldNote`, `proFieldConsent`). Prénom, nom, téléphone, email et consentement sont **verrouillés** (validation Studio : présents une seule fois). Succès → `PageContactProSuccess` remplace la page.
  - Navigation libre entre les étapes (écart voulu au prototype) : tous les onglets (A)–(E) sont cliquables et « Continuer » ne bloque pas. Seul « Envoyer » vérifie tout le dossier : s'il manque quelque chose, le formulaire affiche la 1re étape incomplète et donne le focus à son 1er champ invalide, à chaque tentative, jusqu'à ce que tout soit rempli. Seule cette étape passe en rouge : une étape pas encore affichée par un envoi n'est jamais marquée.
  - Conteneur `ElementsContactForm` : un seul `<form>` (le honeypot en fait partie) qui délègue l'envoi au parcours affiché ; les deux parcours restent montés (`v-show`), donc changer d'onglet ne vide rien.
  - Changement d'onglet animé (GSAP, `useContactSwitchMotion` = timings communs + garde preloader/reduced-motion) : le texte de gauche (`PageContactIntro`, SplitText en lignes, **sans masque**) et les blocs du formulaire montent en s'effaçant, puis les nouveaux apparaissent par le bas. Les hauteurs s'ajustent dès l'échange et doivent finir avant l'arrivée des dernières lignes, sinon sur mobile le texte chevauche le sélecteur d'onglets (aucun `overflow: clip` sur les conteneurs). Chaque changement d'onglet remonte en haut de page via Lenis (`scrollTo(0)`, 1,2 s en `power3.inOut`, verrouillé ; `immediate` sans animation), sinon le texte d'accueil qui change de hauteur pousse le sélecteur hors de l'écran sur mobile. Dans le formulaire, `shownProfile` (parcours visible) ne rejoint `profile` qu'après la sortie, et la page s'en sert pour masquer le texte d'accueil sur mobile aux étapes B–E. Blocs animés un à un : enfants directs de chaque parcours, plus les enfants des conteneurs `data-contact-cascade`.
  - Lien direct `?profil=pro` : lu au setup + `watch` sur la query (Nuxt ≥ 4.4 hydrate une page prérendue sur la route du payload puis restaure la query → pas d'erreur d'hydratation). Changer d'onglet fait `navigateTo({ query }, { replace: true })`, sans remonter la page.
  - Longueurs max des champs texte (deux parcours) = `CONTACT_MAX_LENGTH` (`config/CONTACT_PRO_CONFIG.ts`) : `maxlength` des champs ET contrôle de l'API, pour qu'une saisie trop longue soit bloquée à la frappe plutôt que refusée par le serveur.
  - Envoi pro = `{ answers, questions }` (questions = libellés affichés, servent de repli dans le récap). **L'API relit la config publiée dans Sanity** (`loadProForm`, en FR) : seules les coordonnées + le consentement peuvent faire refuser un dossier (422) ; le reste n'est jamais bloquant, parce qu'une page prérendue peut avoir une config plus ancienne que Sanity. Champ masqué ou choix inconnu → ignoré (log) ; réponse à un champ supprimé → section « Autres réponses » du récap. Chaque champ va dans `Récap dossier PRO` et, s'il a une `airtableColumn`, dans cette colonne (choix → `crmValue` ou libellé FR). Onglets du haut = `contact.profileSwitch`. Détail CRM : `../docs/AIRTABLE.md`.
